...
public:
  ...
  // FIXME Matteo
  const_iterator lower_bound(const_reference x) const {return Impl::lower_bound(x);}
  const_iterator upper_bound(const_reference x) const {return Impl::upper_bound(x);}
};
