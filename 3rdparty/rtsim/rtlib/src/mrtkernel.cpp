...
namespace RTSim {
    ...
    CPU *MRTKernel::getProcessor(const AbsRTTask *t) const
   {
    ...
    // XXX MM, see kernel.hpp:119
    template<class M>
    void setEvtPriorityLevel_map(const int prio_lev, const M& _map)
    {
        for(auto const &it : _map)
        {
            // it.first is the CPU (key)
            // it.second is the evt (value)
            it.second->setPriority(prio_lev + it.second->getPriority());
        }
    }
    void MRTKernel::setEvtPriorityLevel(const int prio_lev)
    {
        setEvtPriorityLevel_map(prio_lev, _beginEvt);
        setEvtPriorityLevel_map(prio_lev, _endEvt);
    }
    ////////////////////////////////////////////////////////////////////////////
